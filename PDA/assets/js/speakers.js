$(document).ready(function() {
    ...
    $.getJSON( "assets/data/speakers.json", function( data ) {
        ...
        $.each( speakersList, function( key, val ) {
            ...
            speakersElem +=         '<h4 class="text-uppercase font-weight-normal mb-0">'+val.fname+' '+val.lname+'</h4>';
            speakersElem +=         '<span>'+val.labelled+'</span>';
            speakersElem +=     '</a>';
            speakersElem += '</li>';
        });
    ...
    $('#speakersModal').on('show.bs.modal', function (event) {
        ...
        modal.find('.pda-speaker').html('<h4>'+speakersList[speakerKey].fname+' '+speakersList[speakerKey].lname+ '</h4>'+speakersList[speakerKey].labelled);
        ...
        modal.find('.pda-venue').text('Venue: ' + speakersList[speakerKey].venue);
